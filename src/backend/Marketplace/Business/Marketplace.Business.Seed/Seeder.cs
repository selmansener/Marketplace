...
namespace Marketplace.Business.Seed
{
    ...
    internal class Seeder : ISeeder
    {
        ...
        public Seeder(ILogger<Seeder> logger, IServiceProvider serviceProvider, SeedCache seedCache, SeedServices seedServices, MarketplaceDbContext dbContext, IHostEnvironment hostEnvironment)
        {
            _logger = logger;
        ...
        public void ClearExecutedServices()
        {
            ...
            using (var stream = assembly.GetManifestResourceStream("Marketplace.Business.Seed.ClearDatabase.sql"))
            using (var reader = new StreamReader(stream, Encoding.UTF8))
            {
                string sql = reader.ReadToEnd();
                if (string.IsNullOrEmpty(sql))
                {
                    throw new InvalidOperationException($"Could not load ClearData file. (Marketplace.Business.Seed.ClearDatabase.sql)");
                }
    ...
    public static class SeedServiceInitializer
    {
        public static void AddSeedServices(this IServiceCollection services, string environment)
        {
            ...
            string resourceName = $"Marketplace.Business.Seed.Data.DataFiles.SeedData.{environment}.json";
